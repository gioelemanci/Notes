...
\begin{enumerate}[label=(\roman*),align=left,itemsep=1.0\baselineskip]
    ...
    \item \textbf{Correct spin-statistics relations}:\\
          ...
          \end{itemize}
          In QM this relations are imposed as an additional postulate, while in QFT it emerges naturally from the commutation relations of field operators. While quantizing a field, one has to define strictly spin-statistics relations, imposing commutation relations for bosonic fields and anticommutation relations for fermionic fields:\footnote{Here these relations seems arbitrary and disconnected, but we will see that the parallelism is strong: the cojugate Dirac field is proportional to the conjugate momenta of the Dirac field, so the only difference is the statistical relation among fields and momenta.}
          \[
              \begin{aligned}
...
\subsection{Units and Scales} \label{sec:units_scales}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[scale = .5]
        ...
        \draw[thick,->] (0,0) -- (0,30) node[anchor=north west]{Energy scale [\si{\giga\electronvolt}]};
