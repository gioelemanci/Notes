...
\[
    \begin{aligned}
        \ddt \hat{\pi} (\mathbf{x}, t) & = \frac{i}{2} [\hat{H}(t),\,\hat{\pi}(\mathbf{x}, t)] = i \left[\int \mathrm{d}^3 \mathbf{y} \, \left( \hat{\pi}^2(\mathbf{y},t) + \vert \nabla_{\mathbf{y}}  \hat{\psi}(\mathbf{y},t) \vert^2 + m^2 \hat{\psi}^2(\mathbf{y},t) \right),\,\hat{\pi}(\mathbf{x}, t)\right]                                                                               \\
...
\begin{figure}[H]
    \centering
    \begin{minipage}{0.5\textwidth}
        We have then to compute the integrals on the imaginary axis, with \(\vert \mathbf{p} \vert = i y\) (and so \(\vert \mathbf{p} \vert \mathrm{d} \vert \mathbf{p} \vert = -y \mathrm{d} y\)) such that
        \[
...
No, the exact solution is given in terms of \textit{modified Bessel functions}, and in the limit for \(r \sim \infty\) then \(D(x-y) \sim e^{-mr} \neq 0\); but we can arrive at the same solution with approximated considerations: all the terms inside the integral are positive, so the integral cannot be zero; furthermore, the exponential term \(e^{-yr}\) suppresses the contributions for large \(y\), so the main contribution comes from \(y \sim m\), but they are nonetheless different from zero, so the whole integral is different from zero.
