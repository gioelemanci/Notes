...
\]
where we have used the clifford algebra of the Pauli matrices \(\sigma^i \sigma^j = \delta^{ij} + i \epsilon^{ijk} \sigma^k\)\footnote{The term proportional to \(\epsilon^{ijk}\sigma^k\) goes to zero due to antisymmetry when contracted with symmetric indices.} and the relativistic dispersion relation \(p_0^2 - \mathbf{p}^2 = m^2\). Thus we can write
\[
    \begin{aligned}
...
\[
    \begin{aligned}
        u_L(\mathbf{p}) = (p_\mu \sigma^\mu) u_R(\mathbf{p}) \frac{1}{m} & = (p_\mu \sigma^\mu) (m A \chi) \frac{1}{m} = A (p_\mu \sigma^\mu) \chi, \\
        \implies u_L(\mathbf{p})                                         & = A (p_\mu \sigma^\mu) \chi,
    \end{aligned}
\]
where \(\chi\) is an arbitrary two-component spinors and \(A\) a normalization constant (thus the second equation is automatically satisfied). Thus the general solution for \(u(\mathbf{p})\) can be written as
\[
