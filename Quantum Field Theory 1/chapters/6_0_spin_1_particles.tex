...
If we now expand the lagrangian density in terms of the potentials, we can exploit the fact that not every component of the vector potential \(A^{\mu}\) is physical, since the EM field is known to have only two physical degrees of freedom (the two polarization states of the photon). Thus the lagrangian density can be rewritten as\TODO{not clear, expand computation}
\[
