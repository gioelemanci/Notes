...
For our purposes, we will not spend time explaining the physical meaning of correlation functions, but rather focus on their definition in the path integral formalism and their computation in simple cases: they are powerful tools to extract physical information from the path integral formulation of quantum mechanics and quantum field theory.

As we will see explicitly, this formalism is equivalent to the canonical quantization approach for scalar theories and thus bosonic fields, and we will compute explicitly correlation functions for the KG field. At the end of the chapter we will include the treatment of fermionic fields, which require some additional tools.

Correlation functions are \textit{normalized averages of the product of n dynamical variables, evaluated at different times and weighted by} \(e^{\frac{i}{\hbar}S}\), and they implement directly the computation of physical observables through the path integral formalism. In our one-dimensional example, the normalized “\(n\)-point correlation function” is defined by
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:n_dimensional_gaussian_integral}
\end{equation}
...
\]
We will check this statement and its meaning in the free theories (next section). It is also useful to define the \textbf{effective action} \(\Gamma[\varphi]\) as the Legendre transform of \(W[J]\) (analogous to the Legendre transform that relates the Lagrangian and Hamiltonian formalisms in classical mechanics)
\begin{equation}
    \Gamma[\varphi] = \min_{J} \{W[J] - J_i \varphi^i\}, \quad \text{where } \varphi^i = \frac{\delta W[J]}{\delta J_i},
    \label{eq:effective_action}
\end{equation}
which is considered as a classical action that includes all quantum corrections. It generates the so-called one-particle irreducible (1PI) correlation functions, though we will not investigate further this particular property. The minimum in \(J\) is obtained at \(\varphi^i = \frac{\delta W[J]}{\delta J_i}\), furnishing a relation \(\varphi^i = \varphi^i(J)\) that must be inverted to obtain \(J_i = J_i(\varphi)\) and inserted back into the right-hand side of \eqref{eq:effective_action} to obtain the effective action indeed as a functional of the variable \(\varphi^i\) only.

The last two functionals, \(W [J]\) and \(\Gamma[\varphi]\), find their main applications in quantum field theory. Equivalent definitions can be given for euclidean path integrals.
...
\]
which produces the linear equations of motion \(K_{ij} \phi^j = 0\):
\[
    \begin{aligned}
        \delta S[\phi] & = - \frac{1}{2} \delta(\phi^i K_{ij} \phi^j) = - \frac{1}{2} \left( \delta \phi^i K_{ij} \phi^j + \phi^i K_{ij} \delta \phi^j \right)                    \\
                       & = - \frac{1}{2} \left( \delta \phi^i K_{ij} \phi^j + \delta \phi^j K_{ji} \phi^i \right) = - \delta \phi^i K_{ij} \phi^j = 0 \quad \forall \delta \phi^i \\
                       & \implies \quad \frac{\delta S[\phi]}{\delta \phi^i} = K_{ij} \phi^j = 0,
    \end{aligned}
\]
since \(K_{ij} = K_{ji}\) is symmetric and the variation \(\delta \phi^i\) is arbitrary. We assume \(K_{ij}\) invertible, which translates to the fact that there are no gauge symmetries in our model.

As an example, consider the harmonic oscillator (with \(m=1\) for simplicity), whose action is
\[
    \begin{aligned}
    ...
    \end{aligned}
\]
where we integrated by parts in the first step and introduced the Dirac delta function \(\delta(t-t^{\prime})\) to expose the “kinetic matrix” \(K(t,t^{\prime}) = \left(\frac{\mathrm{d}^2}{\mathrm{d} t^2} + \omega^2 \right) \delta(t - t^{\prime})\), thus we can write the action in hypercondensed notation as
\[
    S[\phi] = - \frac{1}{2} \phi^i K_{ij} \phi^j,
\]
general for any free theory.

Denoting \(\mathrm{D}\phi = \frac{\mathrm{d}^n \phi}{(-i2\pi)^{n/2}}\), setting \(\hbar = 1\) for simplicity, and using the gaussian result in eq. \eqref{eq:QM_gaussian_integral}, one calculates formally the path integral with sources
\begin{equation}
    Z[J] = \int \mathrm{D} \phi \, e^{\frac{i}{2} \phi^i K_{ij} \phi^j + i J_i \phi^i} = \det(K_{ij})^{-\frac{1}{2}} e^{-\frac{i}{2} J_i (G)^{ij} J_j},
    \label{eq:generating_functional_free_theory}
\end{equation}
where the inverse kinetic matrix \(G^{ij} = (K^{-1})^{ij}\) is the \textbf{Green's function} of the theory, satisfying
\[
...
\]
obtaining for the first few correlation functions (zero, one, and two-point functions)
\[
    \begin{aligned}
        \langle 1 \rangle                     & = 1,              \\
        \langle \phi^{i_1} \rangle            & = 0,              \\
        \langle \phi^{i_1} \phi^{i_2} \rangle & = -i G^{i_1 i_2}.
    \end{aligned}
\]
The first one is a consequence of the normalization, the second one reflects the symmetry \(\phi^i \to -\phi^i\), and the third one is known as the propagator, which we find proportional to the inverse of the kinetic matrix \(K_{ij}\). We did not pay attention to normalization factors since they simplify with \(\frac{1}{Z}\) in this case.

Continuing with the calculation of higher point functions, we see that all correlation functions with an odd number of points vanish, again a consequence of the symmetry \(\phi^i \to -\phi^i\). Those ones with an even number \(n\) factorize into a sum of \((n-1)!!\) terms, given by the product of the 2-point functions which connect any two points in all possible ways. This fact is known as the \textbf{Wick’s theorem}.
\[
    \begin{aligned}
        \langle \phi^{i_1} \phi^{i_2} \phi^{i_3} \rangle            & = 0,                                                                                                                                                                                                                                       \\
        \langle \phi^{i_1} \phi^{i_2} \phi^{i_3} \phi^{i_4} \rangle & = \left(\frac{\hbar}{1}\right)^{4}\left(\frac{i}{\hbar}\right)^2 \left( G^{i_1 i_2} G^{i_3 i_4} + G^{i_1 i_3} G^{i_2 i_4} + G^{i_1 i_4} G^{i_2 i_3} \right)                                                                                \\
                                                                    & = \langle \phi^{i_1} \phi^{i_2} \rangle \langle \phi^{i_3} \phi^{i_4} \rangle + \langle \phi^{i_1} \phi^{i_3} \rangle \langle \phi^{i_2} \phi^{i_4} \rangle + \langle \phi^{i_1} \phi^{i_4} \rangle \langle \phi^{i_2} \phi^{i_3} \rangle, \\
    \end{aligned}
\]
where one recognizes a general pattern to derive all higher point correlation functions for free theories. The 4-point function is given as the sum of the products of all possible pairings of 2-point functions, and is called disconnected since it factorizes into products of lower order correlation functions. This result is summarized in the following theorem.
\begin{theorem}[Wick’s theorem]
    In a free theory with quadratic action \(S[\phi] = -\frac{1}{2} \phi^i K_{ij} \phi^j\), all n-point correlation functions with odd \(n\) vanish, while those with even \(n\) factorize into a sum of \((n-1)!!\) terms, given by the product of the 2-point functions which connect any two points in all possible ways:
    \[
        \langle \phi^{i_1} \phi^{i_2} \cdots \phi^{i_n} \rangle = \sum_{\{(a,b)\}} \prod_{(a,b)} \langle \phi^{i_a} \phi^{i_b} \rangle,
    \]
    where the sum runs over all possible ways to pair the indices \(i_1, i_2, \ldots, i_n\) into pairs \((a,b)\).
\end{theorem}

The \textbf{generating functional of connected correlation functions} \(W[J]\) is obtained from eq. \eqref{eq:generating_functional_free_theory} using the definition in eq. \eqref{eq:generating_functional_connected}:
\[
    W[J] = \frac{1}{2} J_i G_{ij} J_j - \Lambda, \quad \Lambda = -\frac{i}{2} \ln(\det(K_{ij})) = -\frac{i}{2}\Tr (\ln(K_{ij})),
\]
where the constant \(\Lambda\) is an infinite constant that can be regularized and interpreted as the vacuum energy of the free theory. One verifies that it generates a 2-point correlation functions that is connected from\footnote{It also generates a 0-point function, given by \(-\Lambda\), that can be shown to be connected as well.}
\[
    \langle \phi^{i_1} \phi^{i_2} \cdots \phi^{i_n} \rangle_c = \left. \left(-i\right)^n \frac{\delta}{\delta J_{i_n}} \cdots \frac{\delta}{\delta J_{i_2}} \frac{\delta}{\delta J_{i_1}} i W[J] \right|_{J=0}.
\]

It is time to start associating Feynman diagrams to these correlation functions. The 2-point connected correlation function is represented by a line connecting two points (it connects them in the spacetime)
\[
    \langle \phi^{i_1} \phi^{i_2} \rangle_c = -i G^{i_1 i_2} \quad \longrightarrow \quad \feynLine,
\]
and that is why it is called the propagator. Higher point connected correlation functions vanish in a free theory, so there are no other Feynman rules to introduce at this stage. In interacting theories, vertices will appear, connecting more than two points. For example, the four-point correlation function in an interacting theory would be represented by the sum of the three diagrams
\[
    \langle \phi^{i_1} \phi^{i_2} \phi^{i_3} \phi^{i_4} \rangle_c \quad \longrightarrow \quad \feynVert + \feynHoriz + \feynCross \, ,
\]
which correspond to the three possible ways of connecting four points with two propagators; that is why it is said to be non-connected. We will see more Feynman rules when treating interacting theories.

Let us also calculate the \textbf{effective action}. The minimum in \(J\) of eq. \eqref{eq:effective_action} is achieved for
\[
...
\begin{equation}
    ...
    \label{eq:effective_action_free_theory}
\end{equation}
...
\[
    \langle x(t) x(t^{\prime}) \rangle = \left. \frac{1}{Z} \left(\frac{\hbar}{i}\right)^2 \frac{\delta}{\delta J(t^{\prime})} \frac{\delta}{\delta J(t)} Z[J] \right|_{J=0} = -i\hbar (K^{-1})(t, t^{\prime}) = -i \hbar G(t, t^{\prime}),
\]
...
\]
that in a hypercondensed notation would have been written as \(K_{il} G^{lj} = \delta_i^{\ j}\). Adding the Feynman \(i\epsilon\) prescription\footnote{Mathematically, it is seen to arise from requiring that the path integral has a small damping factor \(e^{-\tfrac{i}{\hbar} \int \d{t} x^2(t)}\), obtained by the shift \(\omega^2 \to \omega^2 - i \epsilon\) with \(\epsilon \to 0^+\). Recall the comments made under eq. \eqref{eq:QM_gaussian_integral}} for specifying how to integrate around the poles \(p=\pm \omega\), one computes
\[
...
\begin{figure}[H]
    \begin{minipage}{0.4\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/feynman-prescription.png}
    \end{minipage}
    \hfill
    \begin{minipage}{0.55\textwidth}
        The Feynman prescription sends positive frequencies forward in time and negative frequencies backward in time, corresponding to the physical interpretation of particles and antiparticles with positive energies and always propagating forward in time.

        Having computed the Green's function, we can write the generatoring functional in \eqref{eq:generating_functional_free_theory} we can now complete the square (and reducing to the form in \eqref{eq:QM_gaussian_integral}) to obtain an explicit expression for \(Z[J]\), which will let us compute all correlation functions of the harmonic oscillator.
    \end{minipage}
\end{figure}
Thus defining \(\tilde{x}(t) = x(t) - \int \d{t^{\prime}} G(t, t^{\prime}) J(t^{\prime})\) to complete the square in the exponent of the path integral, we obtain
\[
    \begin{aligned}
        Z[J] & = \int \mathrm{D} x \, e^{i S[x] + i \int \d{t} J(t) x(t)} = \int \mathrm{D} x \, e^{-\frac{i}{2} \int \int \d{t} \d{t^{\prime}} \left( x(t) K(t, t^{\prime}) x(t^{\prime}) + 2 J(t) \delta(t,t^{\prime})x(t^{\prime} ) \right)}                                     \\
             & = \int \mathrm{D} \tilde{x} \exp \left( -\frac{i}{\hbar} \int\int \d{t} \d{t^{\prime}} \tilde{x}(t) K(t,t^{\prime}) \tilde{x}(t^{\prime}) \right) \left[ \exp\left( \frac{i}{2\hbar} \int\int \d{t} \d{t^{\prime}} J(t) G(t,t^{\prime})J(t^{\prime}) \right) \right] \\
             & = Z[0] \exp\left( \frac{i}{2\hbar} \int\int \d{t} \d{t^{\prime}} J(t) G(t,t^{\prime})J(t^{\prime}) \right),
    \end{aligned}
\]
where \(Z[0] = \int \mathrm{D} \tilde{x} \, e^{\frac{i}{\hbar} S[\tilde{x}]}\) is a normalization constant. We can now compute all correlation functions by differentiating with respect to the source \(J(t)\); for example, the two-point function reads
\[
    \begin{aligned}
        \langle x(t)x(t^{\prime}) \rangle & = \left. \frac{1}{Z[0]} \left(\frac{\hbar}{i}\right)^2 \frac{\delta}{\delta J(t^{\prime})} \frac{\delta}{\delta J(t)} Z[J] \right|_{J=0}                                                                                 \\
                                          & = \left. \left(\frac{\hbar}{i}\right)^2 \frac{\delta}{\delta J(t^{\prime})} \frac{\delta}{\delta J(t)} \left[ \exp\left( \frac{i}{2\hbar} \int\int \d{t_1} \d{t_2} J(t_1) G(t_1,t_2)J(t_2) \right) \right] \right|_{J=0} \\
                                          & = -i \hbar G(t,t^{\prime}) = \frac{\hbar}{2\omega} e^{-i\omega \vert t-t^{\prime} \vert}.
    \end{aligned}
\]
...
\]
which is the action of a harmonic oscillator with frequency \(\omega = m\) in hypercondensed notation.

If we complete the square in the exponential of the path integral (with the idea to use gaussian integrals to obtain the generator of correlation functions), we obtain
\[
...
\[
    \langle \phi(x) \phi(y) \rangle = \left. \frac{1}{Z[0]} \left(-i\right)^2 \frac{\delta}{\delta J(y)} \frac{\delta}{\delta J(x)} Z[J] \right|_{J=0} = -i G(x,y),
\]
...
\]
where \(\mathrm{d}^4 p = \d{p_0}\mathrm{d}^3 \mathbf{p}\) and \(p^2 = p_\mu p^{\mu} = -(p_0)^2 + \mathbf{p}^2\). There are poles corresponding to the solutions of the mass shell condition, \(p_0 = \sqrt{\mathbf{p}^2 + m^2}\) (positive energies) and \(p_0 = -\sqrt{\mathbf{p}^2 + m^2}\) (negative energies). The Feynman \(i \epsilon\) prescription sends positive energies forward in time and negative energies backward in time. It corresponds to the physical interpretation of particles and antiparticles with positive energies and always propagating forward in time (thus propagating negative energies backward in time). Setting \(E_p = \sqrt{\mathbf{p}^2 + m^2}\) one finds
\[
    \begin{aligned}
        \langle \phi(x) \phi(y) \rangle & = -i G(x-y) = -i \int \frac{\mathrm{d}^4 p}{(2\pi)^4} \, \frac{e^{-ip_\mu (x^{\mu} - y^{\mu})}}{p^2 + m^2 - i \epsilon}                                                                                                                  \\
                                        & = i \int \frac{\mathrm{d}^3 \mathbf{p}}{(2\pi)^3} \, e^{i \mathbf{p} \cdot (\mathbf{x} - \mathbf{y})}\left[ \frac{\d{p^0}}{2\pi} \frac{e^{-ip^0 (x^0 - y^0)}}{(p^0 - E_p + i\epsilon^{\prime} )(p^0 + E_p - i\epsilon^{\prime})} \right] \\
                                        & = \int \frac{\mathrm{d}^3 \mathbf{p}}{(2\pi)^3} \, \frac{e^{i \mathbf{p} \cdot (\mathbf{x} - \mathbf{y})}}{2 E_p} \left[ \theta(x^0 - y^0) e^{-i E_p (x^0 - y^0)} + \theta(y^0 - x^0) e^{-i E_p (y^0 - x^0)} \right]                     \\
                                        & = \int \frac{\mathrm{d}^3 \mathbf{p}}{(2\pi)^3} \, \frac{e^{i \mathbf{p} \cdot (\mathbf{x} - \mathbf{y})}}{2 E_p} e^{-i E_p \vert x^0 - y^0 \vert}.
    \end{aligned}
\]
Here again the Feynman prescription is crucial to obtain the correct time-ordering in the two-point correlation function as \(\epsilon \sim \epsilon^{\prime} \to 0\), abd by comparing this result with the harmonic oscillator case one sees that the Klein-Gordon field theory is indeed an infinite collection of harmonic oscillators, one for each momentum mode \(\mathbf{p}\), with frequency \(\omega = E_p = \sqrt{\mathbf{p}^2 + m^2}\).
...
\[
    Z_E[J] = \int \mathrm{D} x \, e^{-S_E[x] + \int_{-\beta/2}^{\beta/2} \d{\tau} J(\tau) x(\tau)} \xrightarrow{\beta \to  \infty} \lim_{\beta \to \infty} e^{- \beta E_0(J)},
\]
...
\[
    \begin{aligned}
        Z_E[J] & = \int_{PBC} \mathrm{D} x \, e^{-\frac{1}{2} \int \d{\tau} \, x(\tau) \left( -\frac{\mathrm{d}^2}{\mathrm{d} \tau^2} + \omega^2 \right) x(\tau) + \int \d{\tau} J(\tau) x(\tau)} \\
               & = Z_E[0] \exp\left( \frac{1}{2} \int\int \mathrm{d}\tau \mathrm{d}\tau^{\prime} J(\tau) G_E(\tau - \tau^{\prime}) J(\tau^{\prime}) \right),
    \end{aligned}
\]
from which we get the Green's function in euclidean time, which can be computed from the Fourier transform of the inverse kinetic operator
\begin{equation}
    ...
    \label{eq:euclidean_green_function_harmonic_oscillator}
\end{equation}
