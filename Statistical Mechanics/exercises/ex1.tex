...
\begin{itemize}
    \item \textbf{Case (a)} Free non-relativistic particle:
        \[
        {\cal H} = \sum_{i=1}^N \frac{p_i^2}{2m}
        \]
        where $p_i^2 \equiv (p_i^x)^2$ ($d=1$), $p_i^2 \equiv (p_i^x)^2 + (p_i^y)^2$ ($d=2$), etc.

    \item \textbf{Case (b)} Harmonic oscillator in 1 dimension:
        \[
        {\cal H} = \sum_i \left( \frac{p_i^2}{2m} + \frac{m\omega^2}{2} q_i^2 \right)
        \]
        with $q_i^2 \equiv (q_i^x)^2$, $p_i^2 \equiv (p_i^x)^2$.

    \item \textbf{Case (c)} Free ultra-relativistic particle in 3 dimensions:
        \[
        {\cal H} = c \sum_{i=1}^N |p_i|
        \]
        where $|p_i| \equiv \sqrt{(p_i^x)^2 + (p_i^y)^2 + (p_i^z)^2}$.
\end{itemize}
