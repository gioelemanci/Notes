...
\[
    \mathcal{Z} = \sum_{N=0}^{\infty} \frac{1}{\mathcal{Z}_N} \int_{\mathcal{M}_{\mathcal{S}}} \prod_{i=1}^{N} \frac{\mathrm{d}^d q_i \, \mathrm{d}^d p_i}{h^d} \, e^{-\beta(\mathcal{H}(q_i, p_i) - \mu N)}.
\]
...
\]
where \(\d{\Omega_2} = \frac{1}{h^{dN_2}\,\mathcal{Z}_{N_2}} \prod_{j=1}^{N_2} \mathrm{d}^d q_j^{(2)}\, \mathrm{d}^d p_j^{(2)}\) is the measure on the phase space of the environment.
...
\[
    \sum_{N_1=0}^{\infty}
    \frac{\mathcal{Z}_N}{\mathcal{Z}_{N_1}\mathcal{Z}_{N_2}}
...
\begin{definition}{Grandcanonical average.}
    ...
    \[
        \begin{aligned}
            \langle f(q_i, p_i, N) \rangle_{gc} & = \frac{1}{\mathcal{Z}} \sum_{N=0}^{\infty} \frac{1}{\mathcal{Z}_N} \int \prod_{i=1}^{N} \frac{\mathrm{d}^d q_i \, \mathrm{d}^d p_i}{h^d} \, e^{-\beta(\mathcal{H}(q_i, p_i) - \mu N)} f(q_i, p_i, N) \\
...
It allows us to express quantities such as the pressure or the average particle number as power series that account for the effects of interparticle interactions.

In the dilute limit, the gas behaves as an ideal gas, while higher-order terms in the virial expansion capture corrections due to correlations among pairs, triplets, and larger groups of particles.
...
\[
    ...
    \end{aligned}
\]
Here, \(\mathcal{H}_1 = \frac{p_1^2}{2m}\) contains no interaction term, while \(\mathcal{H}_2 = \frac{p_1^2}{2m} + \frac{p_2^2}{2m} + U_{12}\).
The phase space measure for \( N \) particles is
\[
...
\[
    \mathcal{N} = z \frac{\partial \log \mathcal{Z}}{\partial z}
...
\begin{itemize}
    \item \( B_2(T) \) accounts for pairwise interactions and is directly related to the intermolecular potential \( u(r) \) through
          \[
              ...
              = -\frac{1}{2} J_2(\beta).
          \]
          For repulsive interactions, \( B_2 > 0 \); for attractive interactions, \( B_2 < 0 \).
    \item \( B_3(T) \) describes the effect of three-body correlations and is generally more involved to compute.
\end{itemize}
...
To express the fugacity \( z \) as a function of the density \( n \), we can invert the series:
\[
    \begin{aligned}
...
Hence,
\[
    z = \lambda^3 n - J_2 \lambda^6 n^2 + O(n^3).
\]
...
To connect the virial expansion with a more familiar and physically intuitive equation of state, we now consider a simple yet effective model for the intermolecular potential \( U(r) \).
A convenient choice is the *hard-sphere potential* with an additional attractive tail. In this model, molecules are treated as rigid spheres of diameter \( d \), meaning they cannot overlap; this represents short-range repulsion due to the finite size of particles. At larger separations, an attractive potential \( f(r) < 0 \) is introduced to mimic cohesive forces such as van der Waals or dispersion interactions:
\[
    U(r) =
    \begin{cases}
        \infty & \text{if } r < d,                                 \\
        f(r)   & \text{if } r \geq d, \quad \text{with } f(r) < 0.
    \end{cases}
\]
...
\[
    \begin{aligned}
        J_2(\beta) & = 4\pi \int_0^{\infty} \! \mathrm{d}r\, r^2 \left(e^{-\beta U(r)} - 1\right)                                                            \\
                   & = 4\pi \left[ \int_0^{d} \! \mathrm{d}r\, r^2 (0 - 1) + \int_{d}^{\infty} \! \mathrm{d}r\, r^2 \left(e^{-\beta f(r)} - 1\right) \right] \\
                   & = -\frac{4\pi d^3}{3} + 4\pi \int_{d}^{\infty} \! \mathrm{d}r\, r^2 \left(e^{-\beta f(r)} - 1\right)                                    \\
                   & \doteq -2b + 2\beta a,
    \end{aligned}
\]
where \( a,\, b > 0 \) are constants that characterize the potential.
The first term represents the \emph{excluded volume} due to hard-sphere repulsion, while the second captures the \emph{attractive tail}.
...
\[
    \boxed{\left(p + \frac{a}{v^2}\right)(v - b) = k_B T.}
\]
