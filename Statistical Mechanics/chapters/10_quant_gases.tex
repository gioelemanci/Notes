...
\]
where \(\zeta(3/2)\) is the Riemann zeta function, equal to \(g_{3/2}(1)\).
...
\begin{itemize}
    ...
    \item For \(T < T_{BEC}\), condensation occurs, so we have a macroscopic occupation of the ground state:
          \[
              ...
              \end{dcases}
          \]
\end{itemize}\TODO{insert plot of n0 and nn vs T for Bose gas.}
...
We can now compute the energy per particle \(E/N\) in the two temperature regimes, considering that the total number of particles \(N\) is
\[
    \frac{N}{V} = n_n = \begin{dcases}
        \frac{g}{\lambda_{T}^3} g_{3/2}(z), \quad & T > T_{BEC}, \\
        \frac{g}{\lambda_{T}^3} \zeta(3/2), \quad & T < T_{BEC}.
    \end{dcases}
\]
Thus we have
\begin{itemize}
    \item For \(T > T_{BEC}\):
          \[
              u = \frac{E}{N} = \frac{3}{2} k_B T \frac{g_{5/2}(z)}{g_{3/2}(z)}.
          \]
    \item For \(T < T_{BEC}\):
          \[
              u = \frac{E}{N} = \frac{3}{2} k_B T \frac{\zeta(5/2)}{\zeta(3/2)} \left( \frac{T}{T_{BEC}} \right)^{3/2}.
          \]
\end{itemize}
Note that we have used the expression for the normal component \(n_{n}(T)\) to relate the energy to the total number of particles \(N\): only the normal component contributes to the energy and we are computing the expression for \(u=\frac{E}{N}\frac{V}{V}\) accordingly.

If we consider the expression for the specific heat at constant volume \(c_V\), we can derive it from the energy expression by differentiating with respect to temperature:
\[
    c_V = \left( \frac{\partial u}{\partial T} \right)_V.
\]
Calculating \(c_V\) in the two temperature regimes, we find that it exhibits a discontinuity at the critical temperature \(T_{BEC}\), characteristic of a second-order phase transition, and it goes to zero as \(T\) approaches absolute zero, in accordance with the third law of thermodynamics (fixing the problem we had before with the classical ideal gas).\TODO{insert plot of cV vs T for Bose gas.}
[... CV approximated computation ...]
...
\begin{itemize}
    ...
    \item The grand potential \(\Omega\) is related to the pressure by \(\Omega = - pV\), with \(p\) given by the earlier expression.
    \item The chemical potential \(\mu\) is zero for \(T < T_{BEC}\) and negative for \(T > T_{BEC}\), approaching zero as \(T\) approaches \(T_{BEC}\) from above, while \(N\) is the total number of particles in the system.
\end{itemize}
Substituting these expressions into the entropy formula, we can derive the entropy per particle \(S/N\) in both temperature regimes, revealing how the entropy behaves across the Bose-Einstein condensation transition:
\[
    \begin{dcases}
        \frac{S}{N} & = \frac{5}{2} k_B \frac{g_{5/2}(z)}{g_{3/2}(z)} - k_B \log z, \quad T > T_{BEC},                           \\
        \frac{S}{N} & = \frac{5}{2} k_B \frac{\zeta(5/2)}{\zeta(3/2)} \left( \frac{T}{T_{BEC}} \right)^{3/2}, \quad T < T_{BEC}.
    \end{dcases}
\]\QUESTION{check expressions.}
We can also compute it precisely at the critical temperature \(T = T_{BEC}\):
\[
    \frac{S}{N}\Big\vert_{T=T_{BEC}} = \frac{5}{2} k_B \frac{\zeta(5/2)}{\zeta(3/2)},
\]
while at the critic temperature we have zero particles in the ground state, so all particles contribute to the entropy. As temperature decreases below \(T_{BEC}\), the entropy decreases due to the increasing occupation of the ground state, in which particles stop to contribute to the system entropy, reflecting the onset of macroscopic quantum coherence in the system.
\[
    \Delta s = s(T_{BEC}) - s(0) = \frac{5}{2} k_B \frac{\zeta(5/2)}{\zeta(3/2)} = \frac{\delta Q}{T_c} \neq 0.
\]
Usually PT are induced by interactions among particles, while here we have a PT in an ideal gas: this is possible because of the SSB of the gauge symmetry associated to the phase of the condensate wavefunction, which leads to a new ordered phase with lower entropy. This is a peculiar feature of Bose-Einstein condensation, where the transition is driven by quantum statistics rather than interactions. It is a strange PT indeed, since we have a specific heat discontinuity at \(T_{BEC}\) (typical of second-order PT), but we also have a finite entropy change at the transition (typical of first-order PT). This is a unique characteristic of the Bose-Einstein condensation phenomenon. \QUESTION{is this right?}

\subsection{Supersymmetry Breaking in BE Condensation}
...
\[
    U(\theta) = e^{i \theta \hat{N}} \implies \begin{dcases}
        U(\theta) \hat{a}(\mathbf{k}) U^{\dagger}(\theta) = e^{i\theta} \hat{a}(\mathbf{k}), \\
        U(\theta) \hat{a}^{\dagger}(\mathbf{k}) U^{\dagger}(\theta) = e^{-i\theta} \hat{a}^{\dagger}(\mathbf{k}).
    \end{dcases}
\]
...
\[
    n_0 = \langle \hat{a}_0^{\dagger} \hat{a}_0 \rangle \neq 0 \implies \langle \hat{a}_0 \rangle \neq 0 \text{ for } T < T_{BEC}.
\]
This non-zero expectation value of the annihilation operator \(\hat{a}_0\) indicates that the ground state has a well-defined phase, breaking the global \(U(1)\) gauge symmetry of the hamiltonian. The system chooses a specific phase for the condensate wavefunction, leading to spontaneous symmetry breaking (SSB). This SSB has profound implications for the physical properties of the Bose-Einstein condensate, including the emergence of superfluidity and coherence.\QUESTION{CHECK EVERYTHING}



\section{Further Details on BEC}

We have to acknowledge that when studying the Bose-Einstein condensation as a phase transition, we have made some simplifications and assumptions that may not hold in all situations.
...
\begin{example}[Superfluidity in liquid helium-4]
    In 1938, Allen and Misener, and independently Kapitza, discovered that liquid helium-4 exhibits superfluidity below a critical temperature of approximately \(T_c 2.17 K\). This phenomenon is characterized by the ability of the liquid to flow without viscosity, allowing it to pass through narrow channels and over obstacles without resistance. The superfluid phase of helium-4 is a manifestation of Bose-Einstein condensation, where a significant fraction of helium atoms occupy the same quantum state, leading to macroscopic quantum coherence.
    ...
    We also need a really rarefied gas of helium-4 atoms to observe BEC directly, since in liquid helium-4 the interactions between atoms are strong and complicate the picture. In experiments with ultracold atomic gases, researchers have successfully created Bose-Einstein condensates of helium-4 atoms by cooling them to temperatures on the order of nanokelvins, allowing for the direct observation of BEC phenomena.
\end{example}

\begin{example}[BEC in ultracold atomic gases]
    In 1995, Eric Cornell and Carl Wieman at JILA (University of Colorado) and Wolfgang Ketterle at MIT successfully created Bose-Einstein condensates (BEC) in ultracold atomic gases, marking a significant milestone in experimental physics. They used laser cooling and evaporative cooling techniques to cool a dilute gas of Rubidium-87 atoms (Cornell and Wieman) and Sodium-23 atoms (Ketterle) to temperatures on the order of nanokelvins (\(T_{BEC} \sim \SI{10}{\nano\kelvin} - \SI{1}{\micro\kelvin}\) ), allowing the atoms to occupy the same quantum state and form a macroscopic condensate.
...
\begin{itemize}
    \item \textbf{Laser cooling}: This technique uses the momentum of photons to slow down and cool atoms. By tuning the frequency of laser light slightly below an atomic transition, atoms moving towards the laser absorb photons and experience a recoil that slows them down. This process, known as Doppler cooling, can bring atomic gases down to microkelvin temperatures.
          ...
          We would need to use multiple laser beams arranged in a configuration known as optical molasses to achieve effective cooling in three dimensions. The atoms are trapped in a region where the laser beams intersect, allowing for efficient cooling.

          This technique is often the first step in cooling atoms before applying more advanced methods to reach even lower temperatures, since it would likely reach only microkelvin temperatures.

    \item \textbf{Magnetic evaporation}: This technique involves trapping atoms in a magnetic field and selectively removing the most energetic atoms from the trap. By gradually lowering the depth of the magnetic trap, the hottest atoms escape, leaving behind a colder sample (like evaporation at room temperature with a cup of coffee). This process can be repeated multiple times, lowering further and further the magnetic trap depth to achieve temperatures in the nanokelvin range.
\end{itemize}
...
We can now compute the density of states for photons, starting from the general expression for the density of states in three dimensions: from the computation for the volume in phase space we have
\[
    \Sigma(\epsilon) = g \int_{0\leq c\vert \mathbf{p} \vert \leq \epsilon} \d{p} \frac{V}{(2\pi \hbar)^3} 4\pi p^2 = g \frac{V}{(2\pi \hbar)^3} \frac{4\pi}{3 c^3} \epsilon^3,
\]
which gives
\[
    g(\epsilon) \d{\epsilon} = g \frac{V}{(2\pi \hbar)^3} 4\pi p^2 \d{p},
\]
since it is a spherical volume in momentum space with radius \(p = \epsilon/c\). Differentiating with respect to energy, we find the density of states:
\[
    \omega(\epsilon) = \frac{\partial}{\partial \epsilon} \Sigma(\epsilon) = g \frac{V}{(2\pi \hbar)^3} \frac{4\pi}{c^3} \epsilon^2 = g(\epsilon).
\]
Using this density of states, we can compute the thermodynamic quantities for the photon gas. The average number of photons \(N\) in the system is given by
\[
...
\[
    N = g \frac{V}{(2\pi \hbar)^3} \frac{4\pi}{c^3} \int_0^{\infty} \d{\epsilon} \frac{\epsilon^2}{e^{\beta \epsilon} - 1}.
\]
For the energy \(E\) of the photon gas, we have
\[
    E = \int_0^{\infty} \d{\epsilon} g(\epsilon) \epsilon \langle n(\epsilon) \rangle_{gc}^{B} = g \frac{V}{(2\pi \hbar)^3} \frac{4\pi}{c^3} \int_0^{\infty} \d{\epsilon} \frac{\epsilon^3}{e^{\beta \epsilon} - 1}.
\]
For the grand potential \(\Omega\), we have
\[
    \Omega = - k_B T \int_0^{\infty} \d{\epsilon} g(\epsilon) \log\left( 1 - e^{-\beta \epsilon} \right) = - k_B T g \frac{V}{(2\pi \hbar)^3} \frac{4\pi}{c^3} \int_0^{\infty} \d{\epsilon} \epsilon^2 \log\left( 1 - e^{-\beta \epsilon} \right),
\]
so that the pressure \(p\) of the photon gas is given by
\[
    p = - \frac{\Omega}{V} = k_B T g \frac{1}{(2\pi \hbar)^3} \frac{4\pi}{c^3} \int_0^{\infty} \d{\epsilon} \epsilon^2 \log\left( 1 - e^{-\beta \epsilon} \right).
\]
Thus finally we can compute the equation of state for the photon gas. Notably, since photons are massless particles, they exhibit a relativistic equation of state. The pressure \(p\) and energy density \(u = E/V\) of the photon gas are related by
\[
    p = \frac{1}{3} u.
\]
This relation arises from the fact that photons are ultrarelativistic particles, and it reflects the isotropic nature of the radiation pressure exerted by the photon gas. In order to explixit the temperature dependence of the pressure and energy density, we can evaluate the integrals with a change of variable \(x = \beta \epsilon\):\TODO{check all axpressions of the section.}
\[
    N = g \frac{V}{(2\pi \hbar)^3} \frac{4\pi}{c^3} \left( k_B T \right)^3 \int_0^{\infty} \d{x} \frac{x^2}{e^{x} - 1},
\]
\[
    E = g \frac{V}{(2\pi \hbar)^3} \frac{4\pi}{c^3} \left( k_B T \right)^4 \int_0^{x} \d{x} \frac{x^3}{e^{x} - 1},
\]
\[
    p = k_B T g \frac{1}{(2\pi \hbar)^3} \frac{4\pi}{c^3} \left( k_B T \right)^3 \int_0^{\infty} \d{x} x^2 \log\left( 1 - e^{-x} \right).
\]
The integrals can be evaluated using the properties of the Riemann zeta function and the Gamma function, but we are not interested in the exact numerical factors here. The key point is that both the energy \(E\) and pressure \(p\) scale with the fourth power of temperature.

\subsubsection{Spectral Energy Density}

The \textbf{spectral energy density} \(E(\nu)\) of the photon gas (\(\epsilon = h \nu\)) describes how the energy is distributed among different photon energies (or frequencies) at a given temperature \(T\). It is defined as
\[
    E(\nu,T) \frac{1}{V} = \frac{8\pi}{(hc)^3} \int_0^{h\nu} \d{\epsilon} \frac{\epsilon^3}{e^{\beta \epsilon} - 1}.
\]
Differentiating with respect to frequency \(\nu\), we obtain the spectral energy density per unit frequency:
\[
    u(\nu, T) = \frac{\partial}{\partial \nu} \left( \frac{E(\nu)}{V} \right) = \frac{8\pi h }{c^3} \frac{\nu^3}{e^{\beta h \nu} - 1}.
\]
