...
The \textbf{microcanonical average} of an observable \(f\) over the energy surface \(S_E\) is defined as
\[
...
\]
This describes the long-term average value of the observable along the dynamical evolution of a single system.

\begin{definition}[Ergodic system]
A system is said to be \textbf{ergodic} if, for almost all initial conditions, the time average of any observable equals its microcanonical average:
\[
...
In what follows, we will assume ergodicity, which allows us to replace time averages with ensemble averages when computing thermodynamic quantities.
