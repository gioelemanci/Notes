...
We can now analyze the classical limit of the quantum gas, by stopping at the first order in the power series expansions derived above. This limit corresponds to the regime where quantum effects are negligible, and the behavior of the gas can be described by classical statistics. The classical limit is typically valid when the thermal wavelength \(\lambda_{T}\) is much smaller than the average interparticle distance, which occurs at high temperatures or low densities.
...
\[
    \frac{z^2}{2\sqrt{2}} \pm z - \frac{n \lambda_{T}^3}{g} = 0 \implies z = \sqrt{2} \left( \mp 1 \pm \sqrt{1 + \frac{2^{3/2} n \lambda_{T}^3}{g}}\right),
\]
we solved the second order equation: the first sign is due to Bosonic or fermionic nature, the second to the choice of solution of the quadratic equation. Expanding the square root for small \(n \lambda_{T}^3/g\), since \(z \ll 1\) implies small \(n\), we can use the Taylor expansion of the radical \(\sqrt{1+x} \sim 1 + \frac{x}{2} + \cdots\) for \(x \ll 1\), leading to
\[
    \begin{aligned}
        z & \sim \sqrt{2} \left[ \mp 1 \pm \left( 1 \pm \frac{1}{\sqrt{2}}\frac{n \lambda_{T}^3}{g} - \frac{1}{4} \left(\frac{n \lambda_{T}^3}{g}\right)^2 \right) \right] \\
          & = \frac{n \lambda_{T}^3}{g} \mp \frac{1}{2\sqrt{2}} \left( \frac{n \lambda_{T}^3}{g} \right)^2.
    \end{aligned}
\]
If we now substitute this expression for \(z\) back into the pressure, and keep terms up to second order in \(n \lambda_{T}^3/g\), we find\TODO{check computtion}
\[
    \begin{aligned}
        p & = \frac{g}{\beta \lambda_{T}^3} \left[ \frac{n \lambda_{T}^3}{g} \mp \frac{1}{2\sqrt{2}} \left( \frac{n \lambda_{T}^3}{g} \right)^2 \pm \frac{1}{2^{5/2}} \left( \frac{n \lambda_{T}^3}{g} \right)^2 \right] \\
          & = n k_B T \left[ 1 \mp \frac{1}{4\sqrt{2}} \left(\frac{n \lambda_{T}^3}{g}\right)^2 \right].
    \end{aligned}
\]
This expression represents the semiclassical expansion of the equation of state for a quantum gas, showing how quantum statistics modify the ideal gas law. The correction term \(\mp \frac{1}{4\sqrt{2}} \left(\frac{n \lambda_{T}^3}{g}\right)^2\) indicates that:
\begin{itemize}
...
\end{itemize}
We get the idea of quantum gases being \textit{attractive} or \textit{repulsive} depending on their statistics, leading to different macroscopic behaviors compared to classical gases: it is not a true interaction between particles, we are looking at free gases, but an effective behavior due to the underlying quantum statistics.
...
\subsection{Convergence of the Power Series}
...
\paragraph{Absolute converge.}
We can study the absolute convergence of these series, true for both bosons and fermions, by considering the sum of the absolute values of the terms:
\[
    \sum_{l=1}^{\infty} \left\vert \frac{z^{l}}{l^{\nu}} \right\vert = \sum_{l=1}^{\infty} \frac{\vert z \vert^{l}}{l^{\nu}} \iff \vert z \vert < 1.
\]
For \textbf{fermions} this condition is always satisfied since fermions have \(\mu < \infty \implies z = e^{\beta \mu} \geq 0\). Thus, the series converges absolutely for all physically relevant values of \(z\). The function
\[
    FIGURE-FERMIONS-POWER-SERIES
\]
well defined for all \(z \geq 0\), poitive and continuous and monotonically increasing. For \(z\) near 0 the series assumes a linear behavior, while for large \(z\) it grows more slowly.
We are looking at the limit for small temperatures \(T \to 0\), such that \(\beta \to \infty\) and thus \(z = e^{\beta \mu} \to \infty\) if \(\mu > 0\):\footnote{Remember for the following computation that the discrete internal degeneracy is included in \(A\) or \(g(\epsilon)\).} recalling the expressions for the particle density and energy density
\[
    \begin{aligned}
        n  = \frac{N}{V}       & = A \int_0^{\mu} \d{\epsilon} \epsilon^{1/2} \frac{1}{e^{\beta(\epsilon-\mu)} + 1} = \int_0^{\infty} \d{\epsilon}g(\epsilon)n_{FD}(\epsilon),          \\
        \epsilon = \frac{E}{V} & = A \int_0^{\mu} \d{\epsilon} \epsilon^{3/2} \frac{1}{e^{\beta(\epsilon-\mu)} + 1} = \int_0^{\infty} \d{\epsilon}g(\epsilon)\epsilon n_{FD}(\epsilon),
    \end{aligned}
\]
where it is crucial to know that the \textit{density of states} \(g(\epsilon)\) has unique definitions valid for the system under consideration (here the non-relativistic 3D quantum gas).
Thus it is crucial to understand how \(n_{FD}\) behaves in this limit: we will plot this function to understand its behavior, for \(\beta \to \infty\)
\[
    \frac{1}{e^{\beta(\epsilon - \mu)} + 1} = \begin{dcases}
        1,           & \epsilon < \mu; \\
        \frac{1}{2}, & \epsilon = \mu; \\
        0,           & \epsilon > \mu.
    \end{dcases}
\]
This is the famous \textbf{Fermi-Dirac distribution at zero temperature}, which shows that all states with energy below the chemical potential \(\mu\) are fully occupied, while those above \(\mu\) are empty. The chemical potential at zero temperature is known as the \textbf{Fermi energy} \(E_F\), which defines the highest occupied energy level in the system at absolute zero.\footnote{When the temperature arises, we pass from the Heaviside step function to a smoother curve, which expresses the occupation of the levels above the Fermi energy.}
\[
    FIGURE-FD-DISTRIBUTION
\]
...
\[
    FIGURE-OF-OCCUPATION-LEVELS
\]
We can study how metals behave under these conditions, since the conduction electrons in metals can be modeled as a Fermi gas. At low temperatures, the electrons fill up energy levels up to the Fermi energy, leading to unique electronic properties such as electrical conductivity and heat capacity that differ significantly from classical predictions. We can dope the metal to change the electron density \(n\), which in turn modifies the Fermi energy \(\epsilon_F\) and thus the electronic properties of the material.

We can also compute the grand potential \(\Omega_F\) at zero temperature using the relation \(\Omega_F = - \frac{2}{3} E_F\):
\[
    \Omega_F = - \frac{2}{3} V \epsilon = - \frac{2}{3} V \cdot \frac{2}{5} A \epsilon_F^{5/2} = - \frac{4}{15} V A \epsilon_F^{5/2},
\]
and from this we can derive other thermodynamic quantities such as pressure
\[
    p = - \frac{\Omega}{V} = \frac{4}{15} A \epsilon_F^{5/2} = \frac{2}{5} n \epsilon_F > 0.
\]
For any perfect classical gas we have a zero pressure at zero temperature, while here we have a finite pressure due to the Pauli exclusion principle preventing all fermions from occupying the lowest energy state. This leads to a degeneracy pressure that supports the system against collapse, which is particularly important in astrophysical contexts such as white dwarf stars: the electron degeneracy pressure arising from the Fermi gas of electrons counteracts gravitational collapse, stabilizing the star.
...
\]
which provides a temperature scale below which quantum effects become significant for the fermi gas. At temperatures much lower than \(T_F\), the gas exhibits degenerate behavior, while at temperatures much higher than \(T_F\), it behaves more classically
\[
...
For bosons, the series converges absolutely for \(\vert z \vert < 1\), which corresponds to \(\mu < 0\). However, as \(z\) approaches 1 (i.e., \(\mu\) approaches 0), the series diverges, indicating a breakdown of the power series expansion in this regime.
